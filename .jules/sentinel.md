...
## 2026-04-21 - [Missing JSON Payload Type Validation]
...
## 2023-10-27 - Date Parse DoS Prevention
**Vulnerability:** Converting untrusted/external date strings via `pd.to_datetime` without explicit error handling (e.g., `errors='coerce'`) can raise `DateParseError` or `ValueError`, crashing the data processing pipeline and causing a Denial of Service (DoS).
**Learning:** External data should always be treated as untrusted, especially strings intended for datetime conversion. If malformed strings are passed into unhandled conversion functions, the entire script could fail.
**Prevention:** Always use explicit error handling like `errors='coerce'` when converting untrusted strings to datetime objects in pandas. Then, handle the resulting `NaT` (Not a Time) values gracefully (e.g., by dropping them via `.dropna(subset=['date'])` or providing a safe default) to ensure robustness.
## 2024-05-27 - [Unhandled Numeric Conversion DoS]
**Vulnerability:** Denial of Service (DoS) due to unhandled exceptions when processing corrupted or maliciously modified CSV data.
**Learning:** `src/update_readme.py`, `src/generate_plot.py`, and `index.qmd` read `market_cap` from a CSV assuming it always contained valid numeric values. If the CSV was tampered with to contain non-numeric strings, subsequent operations like sum calculations (`total_market_cap = latest_df['market_cap'].sum()`) or plotting functions (`px.line`) threw unhandled exceptions like `ValueError`, crashing the application and pipeline.
**Prevention:** Always enforce explicit type conversion (e.g., `pd.to_numeric(df['market_cap'], errors='coerce')`) and gracefully handle invalid rows (e.g., `df.dropna(subset=['market_cap'])`) when reading untrusted or externally modifiable data files, even if the file is generated internally.
## 2026-04-30 - [Unhandled NaT Exception in Quarto Dashboard]
**Vulnerability:** Denial of Service (DoS) during the automated build/render process.
**Learning:** In `index.qmd`, the dashboard generated interactive charts by first attempting to load `data/stablecoin_marketcap.csv`. If the file was missing (e.g., deleted or not yet created), `pd.read_csv` would throw a `FileNotFoundError`. If the dataframe was instantiated as empty (or became empty after dropping invalid rows), calling `.max()` on an empty datetime series returns `pd.NaT`. Subsequent string formatting like `latest_date.strftime("%Y-%m-%d")` would crash the script with a `ValueError: NaTType does not support strftime`. These exceptions caused the Quarto site rendering to fail entirely.
**Prevention:** In data pipeline components (like Quarto python blocks or regular scripts), always enforce file existence checks (e.g., `os.path.exists()`) and validate that dataframes have data (`if not df.empty:`) before running operations such as aggregates, sorts, or string formats. Provide a safe fallback (like initializing an empty dataframe and printing a status message) to let the rest of the dashboard render gracefully.
