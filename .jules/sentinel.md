...
## 2024-05-28 - [Quarto Render DoS via Unhandled NaT]
**Vulnerability:** Denial of Service (DoS) causing rendering pipeline crash due to unhandled `FileNotFoundError` and `pd.NaT` values.
**Learning:** `index.qmd` blindly loaded the CSV file without checking if it existed, and assumed that after parsing and dropping invalid dates/data, the dataframe would not be empty. If the file was missing, it caused `FileNotFoundError`. If the file was empty or corrupted (resulting in an empty dataframe), `.max()` on the date column returned `pd.NaT` (Not a Time). Subsequent calls like `latest_date.strftime("%Y-%m-%d")` threw a `ValueError`, which completely crashed the Quarto rendering process.
**Prevention:** When building visualization pipelines or reading data files, always check for file existence (e.g., `os.path.exists()`). Furthermore, handle cases where dataframes may be empty (e.g., `if not df.empty:`) before applying aggregations or formatting operations that cannot handle missing or `NaT` objects.
